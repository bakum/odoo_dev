    ...
    def _compute_record_data(self):
        ...
        for record in self:
            record_data = {
                ...
                'display_name': record.record_name,
                'accessible': record.accessible,
            }
            if record.res_id:
                record_data.update({
                    'id': record.res_id,
                    'url': record._get_record_url(),
                    'data': {}
                })

                # Only include detailed data if accessible
                if record.accessible:
                    ai_thread = record.message_id.thread_id
                    try:
                        # Use AI thread's _model_read method for access control and field filtering
                        record_data_list = ai_thread._model_read(record.res_model, [record.res_id], [])
                        if record_data_list:
                            record_info = record_data_list[0]  # Get the first (and only) record
                            record_data['data'] = record_info
                    except Exception:
                        # Fallback if AI thread access fails; do not raise to avoid system crash
                        pass

            record.record_data = json.dumps(record_data, ensure_ascii=False, indent=2, default=date_utils.json_default)
