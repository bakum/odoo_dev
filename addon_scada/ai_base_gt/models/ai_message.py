...
class AIMessage(models.Model):
    ...
    content_html = fields.Html(string="Content HTML", compute='_compute_content_html', sanitize=False)
    record_reference_ids = fields.One2many('ai.message.record', 'message_id', string="Record References")
    content_full = fields.Text(string="Content Full", compute='_compute_content_full',
                               help="Content including record references data context.")
    func_call = fields.Text(string="Function Call Data")
    func_result = fields.Text(string="Function Call Result")
